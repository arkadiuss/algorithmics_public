...
void printCT(vector<CTNode> &CT){
	loop(i,0,CT.size()){
		ps(i+1);ps(CT[i].ans);ps(CT[i].p);ps(":");
		for(int e: CT[i].chl) {
			ps(e+1);
		}
		entr;
...
void update(vector<TNode> &T, vector<CTNode> &CT, int v) {
	int i = v;
	while(i != -1){
		CT[i].ans = min(CT[i].ans, dist(T,v,i));
		i = CT[i].p;
	}
}

int query(vector<TNode> &T, vector<CTNode> &CT, int v) {
	int i = v;
	int mini = INFTY;
	while(i != -1){
		//ps(i);pln(CT[i].ans);
		mini = min(CT[i].ans + dist(T,v,i), mini);
		i = CT[i].p;
	}
	return mini;
}
