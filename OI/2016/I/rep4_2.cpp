...
//------------------------------------------
pair<ll,ll> ind[3*MAX];
struct betterDiff{
	ll diff;
	ll i,j;
};
bool toSortDiffs(betterDiff a, betterDiff b){
	return a.diff<b.diff;
}
vector<betterDiff> betterDiffs;
...
void coutBettVal(){
	loop(i,betterDiffs.size()-10,betterDiffs.size()){
		cout<<betterDiffs[i].diff<<": ("<<(betterDiffs[i].i)<<","<<(betterDiffs[i].j)<<")\n";
	}
}
betterDiff md(ll diff, ll i, ll j){
	betterDiff bd;
	bd.diff=diff;
	bd.i=i;
	bd.j=j;
	...
	loop(i,1,ASIZE){	
		loop(j,1,i){
			if(a[i]-a[j]>=MAXXTAB&&a[i]-a[j]<MAXVALUE)
				betterDiffs.pb(md(a[i]-a[j],i,j));
		}
					
	}
	sort(betterDiffs.begin(),betterDiffs.end(),toSortDiffs);
}
pair<ll,ll> findBetter(ll x){
	int s=0,e=betterDiffs.size();
	int m;
	while(s<e){
		m=(s+e)/2;
		if(betterDiffs[m].diff==x) return mp(betterDiffs[m].i,betterDiffs[m].j);
		else if(betterDiffs[m].diff<x){
			s=m+1;		
		}else{
			e=m;
		}
